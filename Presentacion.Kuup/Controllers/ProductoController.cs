...
namespace Presentacion.Kuup.Controllers
{
    public class ProductoController : BaseController
    {
        ...
        public ActionResult Json()
        {
            ...
            //var Producto 
            var query = (from q in ClsProductos.getList()
                         select new
                         {
                             q.NumeroDeProducto,
                             q.CodigoDeBarras,
                             q.NombreDeProducto,
                             q.TextoTipoDeProducto,
                             q.TextoMarca,
            ...
            {
                query = query.Where(x => x.CodigoDeBarras.Trim().ToUpper().Contains(searchValue.Trim().ToUpper()) ||
                x.NombreDeProducto.Trim().ToUpper().Contains(searchValue.Trim().ToUpper()) ||
                x.TextoTipoDeProducto.ToString().Trim().ToUpper().Contains(searchValue.Trim().ToUpper()) ||
                x.TextoMarca.ToString().Trim().ToUpper().Contains(searchValue.Trim().ToUpper()));
            }
            ...
            var Producto = query.Skip(skip).Take(pageSize).ToArray();
            return Json(new { draw, recordsFiltered = recordsTotal, recordsTotal, data = Producto }, JsonRequestBehavior.AllowGet);
        }
        #endregion
        #region Alta
        [HttpGet]
